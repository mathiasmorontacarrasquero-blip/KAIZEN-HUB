...
do
        ...
        screenGui.Name = "Kaizen Hub Intro"
        ...
        local kaizenText = Instance.new("TextLabel")
        kaizenText.Size = UDim2.new(0, 320, 0, 130)
        kaizenText.Position = UDim2.new(-0.5, 0, 0.25, 0)
        kaizenText.BackgroundTransparency = 1
        kaizenText.Text = "KAIZEN"
        kaizenText.TextColor3 = Color3.fromRGB(100, 100, 100)
        kaizenText.TextScaled = true
        kaizenText.Font = Enum.Font.GothamBlack
        kaizenText.TextStrokeTransparency = 0.5
        kaizenText.TextStrokeColor3 = Color3.new(0,0,0)
        kaizenText.TextTransparency = 1
        kaizenText.Parent = bg
        ...
        sound.SoundId = "rbxassetid://9045441248"
        sound.Volume = 0.75
        sound.Looped = false
        sound.Parent = _SS
        sound:Play()

        local kaizenTween = TweenInfo.new(0.8, Enum.EasingStyle.Quad, Enum.EasingDirection.Out)
        local kaizenTarget = UDim2.new(0.35, 0, 0.25, 0)
        _TS:Create(kaizenText, kaizenTween, {Position = kaizenTarget, TextTransparency = 0}):Play()
        task.wait(0.35)
        ...
        _TS:Create(kaizenText, fadeOut, {TextTransparency = 1}):Play()
...
-- ===================== NUEVO AUTO STEAL (Irish Hub Style) con bordes negros =====================
...
local function updateTopBar()
    ...
    RunService.RenderStepped:Connect(function()
        ...
        infoLabel.Text = "KAIZEN HUB | Ping: " .. ping .. "ms | FPS: " .. fps
    end)
end

local function setupStealUI()
    local sg = LP.PlayerGui:FindFirstChild("KaizenHubUI")
    if not sg then
    ...
    if not progressBarBg then
        ...
        infoLabel.Text = "KAIZEN HUB | Ping: 0ms | FPS: 0"
        ...
        -- Borde negro para progressBarBg
        local progressBgStroke = Instance.new("UIStroke", progressBarBg)
        progressBgStroke.Color = Color3.fromRGB(0, 0, 0)
        progressBgStroke.Thickness = 1.5
...
local function startAutoSteal()
    setupStealUI()
    if stealHeartbeatConn then return end
...
local function autoSaveConfig()
        ...
        task.delay(0.5, function()
                local cfg = {
                        ...
                        autoStealEnabled=State.autoStealEnabled, grabRadius=STEAL_RADIUS,
                ...
                pcall(function() writefile("KaizenHubConfig.json", HttpService:JSONEncode(cfg)) end)
                saveDebounce = false
        end)
end
...
local function createMobilePanel()
        ...
        local dropBRSetActive = makeSeparateBtn("BtnDropBR", "DROP\nBR",
                UDim2.new(1, COL1_X, BASE_Y, ROW_Y[1]), "KaizenBtn_dropbr.txt",
        ...
        local autoLeftSetActive = makeSeparateBtn("BtnAutoLeft", "AUTO\nLEFT",
                UDim2.new(1, COL2_X, BASE_Y, ROW_Y[1]), "KaizenBtn_autoleft.txt",
                function(setActive)
        ...
        local autoBatSetActive = makeSeparateBtn("BtnAutoBat", "BAT\nAIMBOT",
                UDim2.new(1, COL1_X, BASE_Y, ROW_Y[2]), "KaizenBtn_autobat.txt",
                function(setActive)
        ...
        local autoRightSetActive = makeSeparateBtn("BtnAutoRight", "AUTO\nRIGHT",
                UDim2.new(1, COL2_X, BASE_Y, ROW_Y[2]), "KaizenBtn_autoright.txt",
                function(setActive)
        ...
        local tpDownSetActive = makeSeparateBtn("BtnTpDown", "TP\nDOWN",
                UDim2.new(1, COL1_X, BASE_Y, ROW_Y[3]), "KaizenBtn_tpdown.txt",
                function(setActive, currentActive)
                        tpToGround()
        ...
        local carrySpeedSetActive = makeSeparateBtn("BtnCarrySpd", "CARRY\nSPD",
                UDim2.new(1, COL2_X, BASE_Y, ROW_Y[3]), "KaizenBtn_carryspd.txt",
                function(setActive)
                        toggleSpeedType()
        ...
        local laggerCarrySetActive = makeSeparateBtn("BtnLaggerCarry", "LAGGER\nCARRY",
                UDim2.new(1, COL1_X, BASE_Y, ROW_Y[4]), "KaizenBtn_laggercarry.txt",
                function(setActive)
                        toggleLaggerCarry()
        ...
        local laggerSetActive = makeSeparateBtn("BtnLaggerMode", "LAGGER\nMODE",
                UDim2.new(1, COL2_X, BASE_Y, ROW_Y[4]), "KaizenBtn_laggermode.txt",
                function(setActive)
                        toggleLagger()
...
for _, name in pairs({"JispiHubGUI","VexulHubGUI","KaizenHubGUI"}) do
...
gui.Name="KaizenHubGUI"; gui.ResetOnSpawn=false; gui.DisplayOrder=10
...
local function saveMainPosition()
        ...
        local str = string.format("%.3f,%.1f,%.3f,%.1f", pos.X.Scale, pos.X.Offset, pos.Y.Scale, pos.Y.Offset)
        pcall(function() writefile("KaizenHubGUIPos.txt", str) end)
end
...
local function saveMiniPosition()
        if miniBtn then
                ...
                local str = string.format("%.3f,%.1f,%.3f,%.1f", pos.X.Scale, pos.X.Offset, pos.Y.Scale, pos.Y.Offset)
                pcall(function() writefile("KaizenMiniPos.txt", str) end)
        end
end

local function loadMainPosition()
        local savedPos = nil
        pcall(function() savedPos = readfile("KaizenHubGUIPos.txt") end)
        if savedPos and savedPos ~= "" then
                local parts = {}
                for part in string.gmatch(savedPos, "[^,]+") do table.insert(parts, part) end
                if #parts >= 4 then
...
local function loadMiniPosition()
        local savedPos = nil
        pcall(function() savedPos = readfile("KaizenMiniPos.txt") end)
...
titleLbl.BackgroundTransparency=1; titleLbl.Text="KAIZEN HUB"
...
local radiusBox = makeInputRow("Grab Radius", STEAL_RADIUS, function(v)
        local n = tonumber(v)
        if n and n >= 5 and n <= 300 then
                STEAL_RADIUS = math.floor(n)
        end
end)
setInstaGrab = makeToggleRow("Auto Grab", nil, true, function(on)
...
footerLbl.Text="KAIZEN HUB  ·  v2.0"; footerLbl.TextColor3=Color3.fromRGB(50,50,50)
...
local function loadConfig()
        local hasFile=false; pcall(function() hasFile=isfile("KaizenHubConfig.json") end)
        if not hasFile then return end
        local ok,cfg=pcall(function() return HttpService:JSONDecode(readfile("KaizenHubConfig.json")) end)
        ...
        if cfg.grabRadius and type(cfg.grabRadius)=="number" then
                STEAL_RADIUS=cfg.grabRadius; radiusBox.Text=tostring(cfg.grabRadius) end
        if cfg.autoStealEnabled then State.autoStealEnabled=true; setInstaGrab(true); pcall(startAutoSteal) end
...
-- Auto Steal se inicia automáticamente si está activado
